...
class Model(nn.Module):
    def __init__(self, dataset):
        ...
        self.d = dataset['drug']['data'].size(0)
    ...
    def forward(self, input):
        ...
        torch.manual_seed(0)
        drug_random = torch.randn(self.d, self.OUT)
        drug_random = torch.randn(self.r, self.OUT)

        drug_GCN_1 = torch.relu(self.first_GCN_drug(drug_random.cuda("cuda:0"),
                                input[1]['edge_index'].cuda("cuda:0"),
                                input[1]['data'][input[1]['edge_index'][0],
                                                 input[1]['edge_index'][1]].cuda("cuda:0")))
        drug_GCN_2 = torch.relu(self.second_GCN_drug(drug_GCN_1.cuda("cuda:0"),
                               input[1]['edge_index'].cuda("cuda:0"),
                               input[1]['data'][input[1]['edge_index'][0],
                                                input[1]['edge_index'][1]].cuda("cuda:0")))
        ...
        drug_linear_1 = torch.relu(self.first_linear_drug(drug_GCN_2))
        drug_linear_2 = torch.relu(self.second_linear_drug(drug_linear_1))
        drug = torch.relu(self.third_linear_drug(drug_linear_2))

        drug_linear_1 = torch.relu(self.first_linear_drug(drug_GCN_2))
        drug_linear_2 = torch.relu(self.second_linear_drug(drug_linear_1))
        drug = torch.relu(self.third_linear_drug(drug_linear_2))
        return drug.mm(drug.t())


class encoderLoss(nn.Module):
    def __init__(self, mustlinks, batch_size, gamma):
...
def get_head(out_size, emb):
    x = []
    in_size = out_size
    for _ in range(2):
        x.append(nn.Linear(in_size, 256))
        x.append(nn.BatchNorm1d(256))  
        x.append(nn.ReLU())
        in_size =256
...
class GCL(nn.Module):
    ...
    def forward(self, x, Adj_, args, branch): ###feature anchor
        if branch == 'learner':
...
class MoCo_CDA(GCL):  ###继承了GCL类
    def _build_projector_and_predictor_mlps(self, dim, mlp_dim): 
        hidden_dim = self.encoder_a.projector[0].weight.shape[1]
        del self.encoder_a.projector, self.encoder_l.projector  
...
class Myloss(nn.Module):
    ...
    def forward(self, target, prediction, drug_lap, circ_lap, alpha1, alpha2, sizes):
        loss_ls = torch.norm((target - prediction), p='fro') ** 2
        drug_reg = torch.trace(torch.mm(torch.mm(alpha1.T, drug_lap), alpha1))
        circ_reg = torch.trace(torch.mm(torch.mm(alpha2.T, circ_lap), alpha2))
        graph_reg = sizes.lambda1 * drug_reg + sizes.lambda2 * circ_reg
